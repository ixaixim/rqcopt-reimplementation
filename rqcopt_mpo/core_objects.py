...
# note: field is a way to customize how each attribute is handled by @dataclass machinery. 
#       field allows to avoid mutable default arguments (i.e. two separate Gate instances will not both modify the same empty list)
@dataclass
class Gate:
    ...
    └──────┬──────┘              └──┬──────┬───┘  
       ...
       in  │ (rows)                 │      │      
        ...
        if self.is_single_qubit():
            ...
            if self.tensor.shape == (4, 4): # index: (out, in) (or (row, cols) if you prefer)
    ...
    # Add __hash__ and __eq__ for using Gates in sets/dicts if modifying in place
    def __hash__(self):
        # Simple hash based on object id, assuming gates are not recreated identically
